...
다음에 뭘 할지는 아직 안 정했다. 단순하게 생각하면 Redis가 먼저 떠오른다. 인스턴스마다 따로 노는 메모리를 한 군데로 모으거나, 한쪽에서 발행한 걸 다른 쪽에 전달하면 되지 않나 싶어서다. 이미 쓰고 있는 것이기도 하고.

그런데 그게 맞는 답인지는 모르겠다. 인스턴스를 늘려 본 적이 없으니 뭐가 먼저 깨지는지도 아직 모른다. **안 겪은 문제에 도구부터 고르면, 다음에 비슷한 상황을 만났을 때 그게 정말 필요했는지 판단할 근거가 안 남는다.**

그래서 일단 두 벌 띄워 보려고 한다. 깨지는 걸 보고 나서 정한다.
