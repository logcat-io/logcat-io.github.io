...
하나 배운 차이도 있다. Tomcat NIO 의 논블로킹은 **요청과 요청 사이**의 이야기다. keep-alive 커넥션의
시간은 다음 요청을 기다리는 유휴 구간과, 컨트롤러 코드가 실제로 도는 처리 구간으로 나뉜다. 놀고 있는
유휴 커넥션 수천 개는 Poller 가 스레드 없이 감시해 준다 — 여기까지가 논블로킹이다. 하지만 요청 처리가
시작되면 워커 스레드 하나가 붙고, 컨트롤러가 JDBC 로 DB 응답을 기다리는 동안에도 그 스레드는 묶인 채
응답을 다 쓸 때까지 점유된다. Go 는 처리 중에 대기가 생기면 goroutine 만 재우고 스레드를 회수하지만,
Tomcat 은 처리 중엔 진짜 OS 스레드 하나가 요청 하나에 묶인다.

그래서 `maxThreads`(기본 200)가 동시 처리의 상한이 된다. 요청이 평균 500ms 걸린다면 스레드 하나가
초당 2개, 200개면 초당 400 요청이 이 서버의 천장이다. CPU 를 더 좋은 걸로 바꿔도 안 늘어난다 — 그
500ms 가 CPU 일이 아니라 DB 대기라면 좋은 CPU 는 대기를 못 줄이니까. "느린 DB 쿼리가 스레드 풀을
말린다"는 말이 이 구조에서 나온다.
