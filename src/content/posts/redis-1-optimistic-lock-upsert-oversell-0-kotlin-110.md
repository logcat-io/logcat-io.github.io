...
> 측정 수치는 2026-08 실측 기준입니다. 대조군·커넥션 풀 스윕까지 포함한 전체 측정 기록은 [별도 글](/posts/redis-filter-db-reach-remeasured/)에 있습니다.
...
**0차가 이 설계의 핵심입니다.** 확정 실패할 요청을 DB 앞에서 끊습니다. 차감이 아니라 확인이라 실패해도 되돌릴 게 없고, 원자적 판정은 2차 Lua가 합니다. 0차를 통과했다가 2차에서 실패하는 요청이 생기지만, 0차는 게이트가 아니라 **명백히 안 될 요청을 싸게 걷어내는 자리**입니다.
...
재고 100, 구매 요청 6,000건(피크 초당 1,500 · 3초 유지), 상품 조회 초당 50건을 전 구간 동시에 흘린 시나리오입니다. Redis 재고 키를 크게 덮어 0차·2차 필터만 무력화한 **대조군**을 함께 돌렸습니다.
...
요청 6,000건 중 DB까지 온 것은 500~680건입니다. 재고 100보다 큰 이유는 재고가 마르는 동안 도착한 요청들이 0차를 통과하기 때문입니다.

**커넥션 풀이 실제로 마르는지는 `hikaricp.connections.pending`을 스파이크 동안 200ms 간격으로 샘플링해 확인했습니다.** 대조군은 풀 크기를 10 / 20 / 40으로 바꿔도 모두 100% 포화됐고, 필터가 있으면 어느 크기에서도 마르지 않았습니다.

**풀만 늘리는 것으로는 대체되지 않았습니다.** 10에서 40으로 4배 늘리자 조회 p95가 869ms에서 140ms로 6배 개선됐지만, 필터가 있는 쪽은 2.4ms였습니다. 오히려 버전 충돌이 4,978에서 5,899로 늘었는데, 커넥션을 더 주면 같은 row를 동시에 때리는 요청이 늘어 재시도가 증폭하기 때문입니다.

**이 설계가 값을 하지 않는 구간도 있습니다.** 초당 700 아래에서는 두 구성이 사실상 같습니다. 거부될 요청이 풀을 경합시킬 만큼 많지 않아서, 그 규모에서는 Redis가 "재고의 두 번째 진실"이라는 비용만 남깁니다.

위 절대 수치는 노트북 한 대(k6와 애플리케이션이 CPU를 공유)에서 나온 값이라 다른 환경에서 그대로 재현되지 않습니다. 규모와 무관하게 남는 결론은 **풀 확대가 경합 증폭 때문에 선형으로 먹히지 않는다**는 쪽입니다.

전체 측정 기록은 [별도 글](/posts/redis-filter-db-reach-remeasured/)에 있습니다.
...
대조군 설계, 커넥션 풀 스윕, "풀만 늘리면 되지 않나"에 대한 측정, 이 설계를 안 써도 되는 조건까지 전체 기록을 따로 정리했습니다.
