...
> **2026-08-18 추가.** 이 글을 쓰고 넉 달 뒤에 다시 재봤다. 정합성 얘기는 그대로 맞는데, **"Redis 가 걸러서 DB 엔 재고 수만큼만 온다"는 주장이 틀렸다.** 요청 전량이 DB 를 두 번씩 치고 있었다. 무엇이 틀렸고 왜 그랬는지, 고친 뒤 숫자가 어떻게 바뀌었는지는 [별도 글](/posts/redis-filter-db-reach-remeasured/)에 정리했다. 아래 본문은 당시 기록 그대로 두고, 틀린 곳에만 표시를 달았다.
...
이 글을 쓰고 4개월 뒤에 `pg_stat_user_tables` 로 실제 DB 도달 건수를 세어봤습니다. **9-2절의 "약 100건 수준 유지"가 틀렸습니다.** 요청 5,999건이 전부 DB를 두 번씩 치고 있었습니다.

원인은 Redis 호출 앞에 있던 두 번의 조회였고, 순서를 바꾸고 중복 선검사를 지워 요청당 0.11회로 줄였습니다. 그 과정에서 커넥션 풀이 실제로 마르는 것을 `pending` 으로 관측했고, "풀만 늘리면 되지 않나"도 직접 재봤습니다.

전체 기록은 별도 글로 정리했습니다.

**→ [4개월 뒤 내 글을 다시 측정했다 — Redis 1차 필터는 DB 도달을 막고 있지 않았다](/posts/redis-filter-db-reach-remeasured/)**
