...
description: "타임딜 재고 차감을 Redis 확인 · Optimistic Lock · UPSERT 세 층으로 나눴다. 같은 것을 겹쳐 막는 게 아니라 각 층이 서로 다른 실패를 맡는다. 넉 달 뒤 대조군을 세워 다시 재고, 이 설계를 안 써도 되는 조건까지 확인했다."
...
> 동시성 제어가 필요하니 Redis 를 쓰자는 전제 아래 이미 구현되어 있던 서비스에서 일했습니다. 그 구조 위에 기능을 얹고 운영하면서 Redis 기반 락이 오히려 문제를 만드는 상황을 몇 번 겪었고, **같은 문제를 다시 만난다면 어떤 가정 위에서 DB 를 보호할까**를 직접 만들어 확인해 보기로 했습니다. 운영 시스템을 옮긴 것이 아니라 그 질문에 답하려고 새로 만든 코드입니다.

이 글의 목표는 DB를 보호하면서 타임딜 재고 오버셀과 1인 한도 초과를 막는 구조를 설계·검증하는 것이다. 층은 셋이지만 같은 것을 겹쳐 막는 게 아니라, 각 층이 서로 다른 실패를 맡는다.  
  
단일 Redis + 단일 DB 환경에서, Redis를 락 서버가 아니라 1차 필터로 사용하고, DB를 진실의 원천(single source of truth)로 두는 구조를 실험했다. 그 결과, 재고 100개에 초당 1,500 스파이크를 넣었을 때 Oversell 0건 · 구매 p95 1ms 미만 · DB 도달 요청당 0.11회를 달성했다.

단순히 “락 범위만 잘 잡으면 된다”는 이상적인 전제 대신, Redis/네트워크 장애와 중복 요청까지 고려해 DB 쿼리 레벨 멱등성을 최소 기준으로 잡고 설계한 기록이다.


## 요약

> 측정 수치는 2026-08 실측 기준입니다. 대조군·커넥션 풀 스윕까지 포함한 전체 측정 기록은 [별도 글](/posts/redis-filter-db-reach-remeasured/)에 있다.

-   재고 100개에 초당 1,500 스파이크(요청 6,000)를 넣어 **Oversell 0건 · 5xx 0건**, DB 도달을 **요청당 0.11회**로 묶었다.
-   Redis 재고 확인으로 **확정 실패를 DB 앞에서 걷어내고**, Lua 원자 차감·DB Optimistic Lock·UPSERT WHERE가 각각 재고 게이트·재고 정합성·1인 한도를 맡는다.
...
-   이 글은 성능 튜닝 자체보다, Redis와 DB 사이의 트랜잭션 경계·보상 트랜잭션·정합성 복구로 **DB를 어떻게 보호했는지**에 초점을 둔다. 성능 수치는 이 구조가 “실제 트래픽에서도 버틸 수 있는지”를 검증하기 위한 결과일 뿐, 목적 그 자체는 아니다.
...
재고 100에 요청 6,000건(피크 초당 1,500) 스파이크 환경에서 Oversell 0건과 1인 구매 한도를 동시에 보장하는 타임딜 재고 차감 파이프라인이다.
...
-   동시성 전략 선정 (Optimistic Lock vs Redisson 등)과 계층별 책임 분리 설계
...
<table style="border-collapse: collapse; width: 100%;" border="1" data-ke-align="alignLeft" data-ke-style="style8"><tbody><tr><td>제약</td><td>내용</td></tr><tr><td>Oversell 방지</td><td>재고 100개에 6,000건이 몰려도 정확히 100건만 성공</td></tr><tr><td>1인 한도</td><td>동일 사용자 동시 요청 시에도 maxPerUser 초과 불가</td></tr><tr><td>응답 성능</td><td>사용자는 결과를 거의 즉시 받아야 함 (설계 시 목표 p95 300ms — 실측은 §9 참조)</td></tr><tr><td>가용성</td><td>DB 커넥션 풀이 막히면 다른 도메인 API까지 영향, 격리 필수</td></tr><tr><td>장애 복구</td><td>Redis 장애나 프로세스 크래시 후에도 DB 정합성을 기준으로 자가 복구</td></tr></tbody></table>
...
**0차가 이 설계의 핵심이다.** 확정 실패할 요청을 DB 앞에서 끊는다. 차감이 아니라 확인이라 실패해도 되돌릴 게 없고, 원자적 판정은 2차 Lua가 한다. 0차를 통과했다가 2차에서 실패하는 요청이 생기지만, 0차는 게이트가 아니라 **명백히 안 될 요청을 싸게 걷어내는 자리**다.

## 5-2. 각 방어선 역할

<table style="border-collapse: collapse; width: 100%;" border="1" data-ke-align="alignLeft" data-ke-style="style8"><tbody><tr><td>방어선</td><td>역할</td><td>실패 시</td></tr><tr><td>Redis 재고 확인(0차)</td><td>확정 실패를 DB 도달 전에 차단</td><td>즉시 409, DB 미접근</td></tr><tr><td>DB Optimistic Lock</td><td>재고 음수 방지, Redis–DB 불일치 방지</td><td>Redis 롤백 + 409</td></tr><tr><td>UPSERT WHERE</td><td>maxPerUser 1인 한도 최종 검증</td><td>Redis 롤백 + 409</td></tr></tbody></table>

처음에는 층을 겹쳐 쌓는 것이 안전이라고 생각했는데, 다시 보니 그게 아니었다. 각 층이 막는 것이 서로 다르다 — 0차는 정합성이 아니라 **자원**을 지키고, Optimistic Lock은 재고 음수를, UPSERT는 1인 한도를 지킨다. 하나가 뚫리면 다른 하나가 대신 잡아주는 구조가 아니라는 뜻이다. 진실의 원천은 끝까지 DB고, Redis는 DB에 닿기 전에 확정 실패를 걷어내는 자리다.
...
k6를 이용해 재고 100, 동시 요청 2,000 시나리오를 반복 검증했다. 아래 9-2 는 넉 달 뒤 자원 격리를 보려고 조건을 바꿔 다시 잰 것이라 부하가 다르다.
...
재고 100, 구매 요청 6,000건(피크 초당 1,500 · 3초 유지), 상품 조회 초당 50건을 전 구간 동시에 흘린 시나리오다. Redis 재고 키를 크게 덮어 0차·2차 필터만 무력화한 **대조군**을 함께 돌렸다.

| 지표 | 필터 OFF (대조군) | 필터 ON |
|---|---:|---:|
| DB 도달 / 요청 | 약 6.8회 | **0.11회** |
| 구매 p95 | 144 ~ 203 ms | **0.89 ~ 0.98 ms** |
| 조회 p95 (스파이크 중) | 131 ~ 191 ms | **2.4 ~ 3.2 ms** |
| HikariCP active 최대 | 20 / 20 (포화) | **7 / 20** |
| pending 최대 | **930** | **0** |

요청 6,000건 중 DB까지 온 것은 500~680건이다. 재고 100보다 큰 이유는 재고가 마르는 동안 도착한 요청들이 0차를 통과하기 때문이다.

**커넥션 풀이 실제로 마르는지는 `hikaricp.connections.pending`을 스파이크 동안 200ms 간격으로 샘플링해 확인했다.** 대조군은 풀 크기를 10 / 20 / 40으로 바꿔도 모두 100% 포화됐고, 필터가 있으면 어느 크기에서도 마르지 않았다.

**풀만 늘리는 것으로는 대체되지 않았다.** 10에서 40으로 4배 늘리자 조회 p95가 869ms에서 140ms로 6배 개선됐지만, 필터가 있는 쪽은 2.4ms였다. 오히려 버전 충돌이 4,978에서 5,899로 늘었는데, 커넥션을 더 주면 같은 row를 동시에 때리는 요청이 늘어 재시도가 증폭하기 때문이다.

**이 설계가 값을 하지 않는 구간도 있다.** 초당 700 아래에서는 두 구성이 사실상 같다. 거부될 요청이 풀을 경합시킬 만큼 많지 않아서, 그 규모에서는 Redis가 "재고의 두 번째 진실"이라는 비용만 남긴다.

위 절대 수치는 노트북 한 대(k6와 애플리케이션이 CPU를 공유)에서 나온 값이라 다른 환경에서 그대로 재현되지 않는다. 규모와 무관하게 남는 결론은 **풀 확대가 경합 증폭 때문에 선형으로 먹히지 않는다**는 쪽이다.

전체 측정 기록은 [별도 글](/posts/redis-filter-db-reach-remeasured/)에 있다.
...
이번에는 UPSERT WHERE 한 줄로 1인 한도와 중복효과를 SQL 레벨에서 닫았다.
...
하지만 Redis 1차 필터, Optimistic Lock, UPSERT, 보상 트랜잭션, 정합성 복구까지 실제로 설계·구현·검증해보면서, 적어도 결제/쿠폰/재고 같은 도메인에서는 이 정도 기준을 요구하는 게 과한 게 아니라는 확신이 생겼다. 일반적인 기능에는 오버엔지니어링을 항상 경계한다. 다만 결제·쿠폰·재고처럼 틀리면 되돌리기 어려운 곳에서는, 층을 두껍게 쌓는 게 아니라 **무엇을 어디서 막을지 나눠 두는 것**이 필요하다고 본다. 넉 달 뒤 다시 재보고 나서야 그 구분이 분명해졌다.
...
대조군 설계, 커넥션 풀 스윕, "풀만 늘리면 되지 않나"에 대한 측정, 이 설계를 안 써도 되는 조건까지 전체 기록을 따로 정리했다.

**→ [Redis 1차 필터가 커넥션 풀을 지키는지 대조군으로 검증했다](/posts/redis-filter-db-reach-remeasured/)**
