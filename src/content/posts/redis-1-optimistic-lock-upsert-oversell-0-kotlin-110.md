...
description: "타임딜 재고 차감에 Redis 1차 필터 + Optimistic Lock + UPSERT 3중 방어를 설계했다. 넉 달 뒤 다시 재보니 \"Redis가 DB 도달을 막는다\"는 주장이 틀려 있었다. 무엇이 어긋났고 어떻게 찾았는지까지 기록했다."
...
> **2026-08-18 추가.** 이 글을 쓰고 넉 달 뒤에 다시 재봤다. 정합성 얘기는 그대로 맞는데, **"Redis 가 걸러서 DB 엔 재고 수만큼만 온다"는 주장이 틀렸다.** 요청 전량이 DB 를 두 번씩 치고 있었다. 무엇이 틀렸고 왜 그랬는지, 고친 뒤 숫자가 어떻게 바뀌었는지는 [11절](#11-2026-08-재측정--이-글의-주장-하나가-틀렸다)에 정리했다. 아래 본문은 당시 기록 그대로 두고, 틀린 곳에만 표시를 달았다.

-   타임딜 재고 100개에 2,000 동시 요청을 넣었을 때 **Oversell 0건**을 달성했다.
...
구매 요청
    ...
    ▼ [1차] Redis Lua 원자 차감
  재고 있음 → 통과
  재고 없음 → 즉시 409 (DB 접근 0)   ← 이게 의도였는데 실제로는 아니었다 (11절)
    │
...
<table style="border-collapse: collapse; width: 100%;" border="1" data-ke-align="alignLeft" data-ke-style="style8"><tbody><tr><td>방어선</td><td>역할</td><td>실패 시</td></tr><tr><td>Redis Lua</td><td>초과 요청 대부분을 빠르게 차단, DB 부하 격리</td><td>즉시 409, DB 미접근 <b>(← 11절에서 정정)</b></td></tr><tr><td>DB Optimistic Lock</td><td>재고 음수 방지, Redis–DB 불일치 방지</td><td>Redis 롤백 + 409</td></tr><tr><td>UPSERT WHERE</td><td>maxPerUser 1인 한도 최종 검증</td><td>Redis 롤백 + 409</td></tr></tbody></table>
...
<table style="border-collapse: collapse; width: 100%;" border="1" data-ke-align="alignLeft"><tbody><tr><td>지표</td><td>목표</td><td>결과</td></tr><tr><td>p95 응답 시간</td><td>&lt; 300ms</td><td><b>271ms</b></td></tr><tr><td>Redis 1차 차단율</td><td>재고/요청 외 거부 비율</td><td><b>약 95% (1,900/2,000)</b></td></tr><tr><td>DB 도달 요청</td><td>재고 ± 재시도 범위</td><td><b>약 100건 수준 유지</b> — <b>틀렸다. 실제로는 요청 전량이 DB를 두 번 쳤다 (11절)</b></td></tr><tr><td>DB 커넥션 풀 고갈</td><td>없음</td><td><b>없음</b></td></tr></tbody></table>

Redis 1차 필터 덕분에 DB는 재고량에 비례하는 소수의 요청만 처리하면 되도록 **설계했다.** 그런데 넉 달 뒤에 `pg_stat_user_tables` 로 실제 도달 건수를 세어보니 그렇지 않았다. 이 문단이 이 글에서 가장 크게 틀린 곳이다 — 자세한 건 11절.

p95 271ms 와 차단율 95% 는 당시 k6 출력에서 옮긴 값인데, 원본 로그를 남겨두지 않아 지금 재현할 수 없다. 그래서 이 두 수치는 근거 있는 값으로 취급하지 않는다.
...
## 11. 2026-08 재측정 — 이 글의 주장 하나가 틀렸다

넉 달 뒤에 이 프로젝트를 다시 열었다. 처음엔 부하 테스트 시나리오가 마음에 안 들어서였다. 재고 100에 2,000을 넣은 건 맞는데 왜 2,000인지 근거가 없었고, 무엇보다 **"타임딜이 다른 요청을 밀어낸다"는 게 이 설계의 전제인데 측정에 밀려나는 쪽이 없었다.** 구매 요청만 쏘고 있었다.

그래서 상품 조회를 초당 50건씩 전 구간 흘리고, 중간에 타임딜 스파이크를 얹었다. 그리고 Redis 재고 키를 크게 덮어 1차 필터만 무력화한 대조군을 만들었다. 코드는 한 줄도 안 고쳤다.

### 11-1. DB 도달은 "약 100건"이 아니었다

먼저 `pg_stat_user_tables` 로 실제 도달 건수를 셌다. 재고 100, 구매 요청 5,999건.
...
**요청 5,999건이 전부 DB를 두 번씩 쳤다.** 재고 소진으로 거부될 5,899건도 예외 없이.

9-2절에 "약 100건 수준 유지"라고 적어둔 게 그냥 틀렸다. 재본 적 없이 설계 의도를 결과처럼 쓴 것이다.

### 11-2. 주석은 맞았는데 흐름이 틀렸다

`StockDecreaser` 에는 4월에 내가 직접 이렇게 적어뒀다.
...
이 주석은 거짓이 아니다. `decrease()` 안에서는 정말로 Redis 실패 시 DB를 안 친다. 문제는 그 함수가 불리기 **전에** 있었다.
...
**파일 하나만 읽으면 맞고, 흐름 전체를 보면 틀린 상태였다.** 주석의 사정거리는 그 파일까지다.

다르게 말하면, 의도를 주석으로 남겨뒀기 때문에 넉 달 뒤에 어긋난 걸 찾을 수 있었다. 기준이 없으면 `1.02회`라는 숫자를 보고도 "원래 이런 건가" 하고 넘어갔을 것이다.

### 11-3. 두 가지를 고쳤다

**Redis 재고 확인을 맨 앞으로 옮겼다.** 차감이 아니라 `GET` 이라 실패해도 되돌릴 게 없다. 롤백 경로가 늘지 않는다. 원자적 판정은 여전히 뒤쪽 Lua 가 한다 — 앞의 확인을 통과했다가 Lua 에서 실패하는 요청이 생기는데, 그건 결함이 아니라 필터의 성격이다. 이 자리는 게이트가 아니라 **명백히 안 될 요청을 싸게 걷어내는 곳**이다.

**`getPurchasedQuantity` 선검사를 지웠다.** 4-4절에서 UPSERT WHERE 로 1인 한도를 원자적으로 닫아뒀는데, 그 앞에 굳이 SELECT 로 한 번 더 확인하고 있었다. 정확성에 필요 없는 코드였다. 재구매자 한 명의 Redis 롤백을 아끼려고 **모든 요청**이 DB를 한 번 더 치는 거래였던 셈이다.

같은 사용자가 동시에 20회 시도해도 정확히 1건만 성공하고 Redis·DB 재고가 일치하는 것을 통합 테스트로 확인했다. 선검사 없이도 UPSERT 가 한도를 지킨다.
...
**DB 왕복 18배, 구매 응답 11배.** 요청 6,000건 중 DB까지 온 것은 500~680건이다. 애초 기대는 "재고 + 조금"이었으니 그보다는 크다 — 재고가 마르는 동안 도착한 요청들이 확인을 통과하기 때문이다. 그래도 요청 수 기준으로는 9%다.

### 11-4. 그래서 무엇이 지켜졌나 — 커넥션 풀
...
|---:|---|---:|---:|---:|---:|
| **10** | 필터 ON | 2.1 ms | 1 / 10 | **0** | 0 |
| | 필터 OFF | 869 ms | 10 / 10 | **930** | 4,978 |
| **20** | 필터 ON | 2.7 ms | 7 / 20 | **0** | 0 |
| | 필터 OFF | 323 ms | 20 / 20 | **370** | 5,574 |
| **40** | 필터 ON | 2.4 ms | 3 / 40 | **0** | 0 |
| | 필터 OFF | 140 ms | 40 / 40 | **267** | 5,899 |

**대조군은 모든 풀 크기에서 100% 포화되고 대기열이 930까지 쌓인다. 필터가 있으면 어느 크기에서도 마르지 않는다.** 최대 7개를 썼다.

이제 "커넥션 풀을 지킨다"는 말이 추론이 아니라 관측이 됐다.

### 11-5. 풀만 늘리면 되지 않나

이 설계에 대한 가장 자연스러운 반론이라 직접 재봤다.

풀을 10에서 40으로 4배 늘리자 869ms 가 140ms 로 **6배 개선됐다. 완화는 된다.** 그런데 같은 조건에서 필터가 있는 쪽은 2.4ms 다. **4배 늘린 풀이 여전히 60배 느리다.**
...
**풀을 키울수록 버전 충돌이 늘었다.** 커넥션을 더 주면 같은 row 를 동시에 때리는 요청이 늘고, 충돌이 늘면 재시도가 늘고, 재시도가 늘면 왕복이 는다. 늘린 자원의 일부가 경합 증폭으로 되돌아온다. 10-2절에서 "단일 row 에 쓰기 경합이 몰린다"고 적어둔 패턴이 여기서 다시 나온다.

### 11-6. 언제 값을 하고 언제 안 하나
...
| 초당 3,000 | 편차 20배 | 1,613 ~ 2,122 ms |

**초당 700 아래에서는 두 구성이 사실상 같다.** 거부될 요청이 풀을 경합시킬 만큼 많지 않다. 이 구간에서 Redis 는 이득 없이 "재고의 두 번째 진실"이라는 비용만 남긴다. 이 설계를 **안 써도 되는 조건**이 여기다.

초당 3,000은 결론에서 뺐다. 양쪽 다 무너지고 필터 ON 의 편차가 20배다. k6와 애플리케이션이 같은 머신에서 CPU를 나눠 쓰는 한계라, 이 구간 숫자로는 아무 말도 하지 않기로 했다.

정합성은 전 구간에서 유지됐다. 초당 3,000·필터 OFF 로 12,000 요청을 몰아도 성공은 정확히 100건이고 5xx 는 0이다. **이 글에서 정합성에 대해 쓴 내용은 전부 그대로 유효하다.**

### 11-7. 재는 김에 걸린 두 가지

부하를 재려다 엉뚱한 걸 발견했다.

**API로 만든 딜은 한 개도 팔 수 없었다.** `TimeDeal.create()` 는 항상 `SCHEDULED` 로 만드는데 `ACTIVE` 로 바꾸는 코드가 어디에도 없었다. `isActiveAt()` 은 `status == ACTIVE` 를 요구한다. 첫 측정에서 구매 2,099건이 통째로 400 으로 떨어진 원인이 이거였고, 이전 측정들은 SQL 로 상태를 직접 올려서 돌린 것이었다.
...
`STOCK_VERSION_CONFLICT` 로 분리하고 시도 횟수를 실어 보내게 고쳤다. 이걸 고치기 전에는 11-4절의 충돌 열을 만들 수 없었다. 충돌 횟수를 알려면 Redis `INCRBY` 호출 수를 세는 우회로가 필요했다. **관측 가능성은 대시보드가 아니라 실패를 표현하는 타입에서 시작한다.**
...
넉 달 뒤에 그 도구가 이 글의 주장 하나를 깼다. 성능을 재려던 게 아니라 시나리오가 마음에 안 들어서 다시 짰을 뿐인데, 그 과정에서 의도와 구현이 어긋난 지점, 딜이 절대 안 팔리는 버그, 실패 코드가 정반대 신호를 주던 것까지 같이 나왔다.

그래서 이번에 하나 더 붙이자면 — **만든 것이 의도대로 동작하는지는, 의도를 적어두고 나중에 직접 재봐야 안다.** 코드는 컴파일되고 테스트는 통과하고 정합성도 맞았지만, 정작 이 설계의 존재 이유였던 "DB 도달 차단"은 처음부터 지켜지지 않고 있었다.
