---
title: "[SPRING] DB 개인정보 암호화 시스템 구축기: AES‑256‑GCM + Blind Index"
description: >-
  전화번호, 빌링키 같은 민감한 데이터를 DB에 저장해야 하는 순간이 온다. 개인정보보호법이나 정보통신망법을
  따져보면 양방향 암호화가 필수인 항목이 꽤 많다. 직접 설계하고 구현해 봤는데, 생각보다 고려할 것이 많았다.
...
> 전화번호, 결제 정보 같은 민감 데이터를 DB에 넣어야 하는 일을 맡으면서 시작됐다. 단순히 암호화만 하면 되는 줄 알았는데, 검색은 어떻게 하지? 마이그레이션은? NestJS 서버와 같은 DB를 쓰는데 호환은? 하나씩 해결해 나간 과정을 정리한다.

## 들어가며

전화번호, 빌링키 같은 민감한 데이터를 DB에 저장해야 하는 순간이 온다. 개인정보보호법이나 정보통신망법을 따져보면 양방향 암호화가 필수인 항목이 꽤 많다. 직접 설계하고 구현해 봤는데, 생각보다 고려할 것이 많았다.

> 실무에서 마주친 제약을 도메인과 식별 정보만 지우고 다시 구성했다. 암호화 스킴과 키 운용을 그대로 적는 건 그 자체가 위험해서, 어떤 제약이 서로를 막았고 무엇을 골랐는지만 남긴다.
...
가장 고민했던 부분이다. 전화번호를 다루는 서비스 코드가 92곳이 넘었는데, 각 곳에서 `encrypt()`/`decrypt()`를 직접 호출하면 대규모 수정이 필요하고 누락 위험도 컸다.

**JPA `@Convert`를 선택한 이유:**

| 방식 | 장점 | 단점 |
| --- | --- | --- |
| 서비스 레이어 수동 호출 | 명시적 | 92곳+ 수정 필요, 누락 시 평문 저장 버그 |
...
전화번호를 다루는 서비스 코드가 92곳 이상이었다. 각 곳에서 `encrypt()`/`decrypt()`를 직접 호출하는 방식은 대규모 수정 + 누락 위험이 있었다. `@Convert` 한 줄이면 서비스 코드 수정 없이 자동 암복호화가 된다. 대신 Hibernate가 Converter를 직접 `new`로 생성하므로 `@Autowired` 주입이 불가능한 문제가 있었다. 이를 `CryptoServiceHolder` 정적 홀더 패턴으로 해결했다:
...
2.  **JPA Converter의 위력.** 서비스 코드 92곳을 수정하는 대신 엔티티에 `@Convert` 한 줄 추가하는 것으로 해결했다. 다만 Hibernate가 Converter를 직접 new로 생성하는 문제 때문에 정적 홀더 패턴이 필요했다.
...
암호화 모듈을 직접 만들어 보면서 NIST 표준 문서, GCM의 내부 동작, IV 충돌 확률 계산 같은 것을 파고들게 됐다. 라이브러리가 해 주는 부분과 내가 정해야 하는 부분이 어디서 갈리는지는 직접 만들어 봐야 보였다.
